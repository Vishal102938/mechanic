...
const RejectedRequests = () => {
  ...
  const fetchRequests = async () => {
    ...
    const rejected = result.data.filter(req => req.status === "rejected");

    rejected.sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt));
    ...
    <div className="w-full min-h-screen bg-gradient-to-r from-[#0f2027] via-[#203a43] to-[#2c5364] text-white">
      ...
      <div className="hidden md:block md:ml-[20%] p-8 mt-[70px]">
        <h1 className="text-4xl font-extrabold mb-8 
          bg-gradient-to-r from-red-500 via-pink-400 to-orange-500 
        ...
        {requests.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {requests.map(req => (
              <div
                key={req._id}
                className="bg-red-50 text-gray-900 rounded-xl shadow-lg p-6 
                transform hover:scale-105 transition duration-500 ease-in-out border-l-4 border-red-400"
              >
                <h2 className="text-2xl font-bold mb-3 text-red-700">{req.name}</h2>
                ...
                <p className="mt-3 text-sm text-gray-600">
                  <span className="font-semibold text-pink-600">📅 Rejected At:</span>{" "}
      ...
      <div className="block md:hidden p-4 mt-[70px]">
        <h1 className="text-2xl font-extrabold mb-6 bg-gradient-to-r from-red-500 via-pink-400 to-orange-500 
        ...
        {requests.length === 0 ? (
          ...
          <ul className="space-y-3">
            {requests.map(req => (
              <li key={req._id} className="bg-red-50 text-gray-900 rounded-lg shadow-md">
                <div
                  onClick={() => setOpenId(openId === req._id ? null : req._id)}
                  className="cursor-pointer px-4 py-3 hover:bg-red-100 flex justify-between items-center rounded-lg transition duration-300"
                >
                  <span className="font-semibold text-red-700">{req.name}</span>
                  <span className="text-red-500">{openId === req._id ? "▲" : "▼"}</span>
                </div>

                {openId === req._id && (
                  <div className="mt-2 bg-white p-4 rounded-lg text-sm space-y-1 animate-fadeIn">
                    <h2 className="text-lg font-bold mb-2 text-red-700">{req.name}'s Request</h2>
                    <p><span className="font-semibold text-pink-600">📱 Mobile:</span> {req.mobile}</p>
                     <p><span className="font-semibold text-indigo-600">🏠 House No:</span> {req.houseNumber}</p>
                    <p><span className="font-semibold text-teal-600">🛣️ Street:</span> {req.street}</p>
                    {req.landmark && (
